...
//#include <direcconio.ht.h>		// For mkdir()
#include <vector>
#include <string>
//#include <string.h>
#include <opencv/cv.h>
...
void saveArffFile(const char* filename, CvMat* projectedFaceMat);
void test(char *szFileTest);
...
int mainpca(int argc, char** argv) {
  ...
  if (argc >= 2 && strcmp(argv[1], "train") == 0) {
    ...
    learn(szFileTrain);
  } else if (argc >= 2 && strcmp(argv[1], "regc") == 0) {
    char *szFileTest;
    if (argc == 3)
...
void saveArffFile(const char* filename, CvMat* projectedFaceMat) {
  ...
  char arffFile[255];
  sprintf(arffFile, "%s.arff", filename);
  ofstream ofdata;
