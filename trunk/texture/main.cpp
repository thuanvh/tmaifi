...
int main(int argc, char** argv) {
  try {
    ...
    if (typeOfFunction == FUNC_LEARNING) {
      ...
      testing(refOutFilePath, refFilePath, numberNeighbor);

    } else if (typeOfFunction == FUNC_CROSS_TEST) {

      crossTesting(histoName, crossTestPercent, numberNeighbor);

    } else if (typeOfFunction == FUNC_SEGMENT) {
      segmenter(refRefFilePath, histoName, num_color, numberNeighbor, segmblocksize);
    } else if (typeOfFunction == FUNC_SEGMENT_CAH) {
      segmenterCAH(refRefFilePath, histoName, num_color, numberGroup, segmblocksize);
    }
  } catch (const char* e) {
    // error
