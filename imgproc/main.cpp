...
int main(int argc, char** argv) {
  try {
    ...
    IplConvKernel* ele = cvCreateStructuringElementEx(erodesize, erodesize, 2, 2,
            CV_SHAPE_CUSTOM, cvArrayClose);


    cvDilate(imgMask, imgTransform,ele, 1);
    cvErode(imgTransform, imgTransform, ele, 1);
    cvMorphologyEx(imgTransform, imgTransform, imgTemp, ele, CV_MOP_OPEN);
